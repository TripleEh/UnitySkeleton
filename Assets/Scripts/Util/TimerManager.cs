...
// Unity's Time system is pretty limited, and in some instances scaling internal time can 
// break things like animation playback and UI interactions. 
//
			...
			foreach (UInt64 iKey in aKeys)
			{
				if (iKey <= iTime)
				{
					if (m_aEventTable.TryGetValue(iKey, out d))
					{
						...
						{
							Debug.LogWarning("Invoke called on null target");
						}
					}
